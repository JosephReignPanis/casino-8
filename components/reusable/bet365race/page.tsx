...
const Bet365Race: React.FC<Bet365RaceProps> = ({
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-zinc-800 p-2 mt-2 rounded-sm">
        <button className="flex py-2 px-4 text-center border border-amber-400 text-amber-400 rounded-sm justify-center items-center">
          Refresh
        </button>
      </div>
      {/* Match Information */}
      <div className="bg-zinc-800 p-2 mt-2 rounded-sm">
        <div className="flex flex-row justify-between bg-zinc-900 border border-b-zinc-900 border-zinc-950 rounded-sm">
        ...
        {odds.map((item, index) => (
          <div
            key={index}
            className={`grid grid-cols-2 sm:grid-cols-4 w-full gap-2 border p-1 rounded-sm border-neutral-950 ${
              index === 0
                ? "mt-2 bg-zinc-900"
                : index === 1
                ? "bg-zinc-900"
          ...
          >
            {/* Left Column */}
            <div className="flex flex-row  p-2 items-center gap-2 justify-between border border-zinc-800 rounded-sm">
