...
const SnifflingBoard: React.FC<SnifflingBoardProps> = ({
  ...
  return (
    <>
      ...
      {/* Content Section */}
      <div className="w-full bg-neutral-800 p-2 rounded-sm my-2">
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between w-full bg-zinc-900 p-2 items-center gap-2 rounded-sm">
        ...
        {categoryrows.map((categoryRow, index) => (
          <div key={index}>
            ...
            {/* Option Cards */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 w-full bg-zinc-900 p-2 rounded-sm">
              {categoryRow.options.map((option, optionIndex) => (
                <div
                  key={optionIndex}
                  className="flex flex-col items-center gap-2 bg-zinc-800 rounded-sm overflow-hidden w-full"
                >
